...
def main(args):
    ...
    with open("/mnt/data132/taozhen/LLMopen_Benchmark/Detector/XLNet/Chinese/dataset/train.json", 'r') as f:
        train_data = json.load(f)
    trainloader = MyDataLoader(dataset=datapre(jsondata = train_data, tokenizer=tokenizer),
                               batch_size=args.batch_size, shuffle=True)

    with open("/mnt/data132/taozhen/LLMopen_Benchmark/Detector/XLNet/Chinese/dataset/val.json", 'r') as f:
    ...
    for epoch in tqdm(range(args.resume_epoch, args.nepochs)):
        ...
        for step, sample_batched in enumerate(valloader):
            ...
            if step % num_iter_ts == num_iter_ts - 1:
                ...
                cur_f = mean_valAcc
                if cur_f > best_f:
                    save_path = os.path.join(save_dir, 'origin_xlnet_ch' + '.pth')
