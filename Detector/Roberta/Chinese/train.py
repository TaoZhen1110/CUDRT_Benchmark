...
from data_prepare import datapre, MyDataLoader
...
def get_arguments():
    ...
    parser.add_argument("-checkpoint", type=str,
                        default='/mnt/data132/taozhen/LLMopen_Benchmark/Detector/Roberta/Chinese/robert_pretrained/',
...
def main(args):
    ...
    with open("/mnt/data132/taozhen/LLMopen_Benchmark/Detector/Roberta/Chinese/dataset/train.json", 'r') as f:
        train_data = json.load(f)
    trainloader = MyDataLoader(dataset=datapre(jsondata = train_data, tokenizer=tokenizer),
                               batch_size=args.batch_size, shuffle=True)

    with open("/mnt/data132/taozhen/LLMopen_Benchmark/Detector/Roberta/Chinese/dataset/val.json", 'r') as f:
        val_data = json.load(f)
    valloader = MyDataLoader(dataset=datapre(jsondata = val_data, tokenizer=tokenizer),
    ...
    ##################  Model ###################
    model = Roberta_model(checkpoint=args.checkpoint, freeze=args.freeze)
    ...
    for epoch in tqdm(range(args.resume_epoch, args.nepochs)):
        ...
        for step, sample_batched in enumerate(valloader):
            ...
            if step % num_iter_ts == num_iter_ts - 1:
                ...
                cur_f = mean_valAcc
                if cur_f > best_f:
                    save_path = os.path.join(save_dir, 'origin_roberta_ch' + '.pth')
